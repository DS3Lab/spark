...
class ModelAveragingSGDShuffle private[spark] (private var gradient: Gradient, private var updater: Updater)
  ...
  def optimize(data: RDD[(Double, Vector)], initialWeights: Vector): Vector = {
    val (weights, _) = ModelAveragingSGDShuffle.runMiniBatchSGD(
      data,
      gradient,
      updater,
      stepSize,
...
/**
  ...
  */
@DeveloperApi
object ModelAveragingSGDShuffle extends Logging {
  /**
  ...
  def runMiniBatchSGD(
    ...
    val weights_final: DenseVector = repeatMLStart(data,
      bcWeights, numIterations, stepSize, regParam, num_features, numExamples, gradient, updater)

    bcWeights.destroy() // destory the broadcast variable

    val train_loss = data.map(x => gradient.computeLoss(x._2, x._1, weights_final))
      .reduce((x, y) => x + y)
    val regVal = updater.getRegVal(weights_final, regParam)
    logInfo(s"ghandTrainLoss=regVal:${regVal}=" +
      s"trainLoss:${(train_loss) / numExamples + regVal}")
  ...
  def repeatML(dataRDD: RDD[(Double, Vector)], initModelRDD: RDD[DenseVector],
               numIter: Int, stepSize: Double, regParam: Double, numFeatures: Int, numExamples: Long,
               gradient: Gradient, updater: Updater): DenseVector = {

    if (numIter > 0) {
      if (TaskContext.isDebug) {
        val weights_tmp = initModelRDD.take(1)(0)
        val train_loss = dataRDD.map(x => gradient.computeLoss(x._2, x._1, weights_tmp))
          .reduce((x, y) => x + y)
        val regVal = updater.getRegVal(weights_tmp, regParam)
        logInfo(s"ghandTrainLoss=regVal:${regVal}=" +
          s"trainLoss:${(train_loss) / numExamples + regVal}")
      ...
      val models: RDD[DenseVector] = updateModel(dataRDD, initModelRDD, stepSize, regParam, gradient, updater)
      ...
      repeatML(dataRDD, averagedModels, numIter - 1, stepSize, regParam, numFeatures, numExamples, gradient, updater)
    }
    else {
  ...
  def repeatMLStart(dataRDD: RDD[(Double, Vector)], bcWeights: Broadcast[DenseVector],
    ...
    if (numIter > 0) {
      ...
      // and the corresponding model
      val models: RDD[DenseVector] = updateModelStart(dataRDD, bcWeights, stepSize, regParam, gradient, updater)
      // models has numPartitions partitions, each partition with one Iterator,
      // and the Iterator has only one element, U, which is the local model.
      val averagedModels: RDD[DenseVector] = allReduce(models, numFeatures)
      // here exchange the model, do something like a all-reduce, to communicate the model,
  ...
  /**
    * @param dataRDD the input data RDD
    * @param initModelRDD the model
    * @param stepSize
    * @param regParam
    * @param gradient
    * @param updater
    * @return
    */
  def updateModel(dataRDD: RDD[(Double, Vector)], initModelRDD: RDD[DenseVector], stepSize: Double, regParam: Double,
                  gradient: Gradient, updater: Updater): RDD[DenseVector] = {
      ...
      (itd: Iterator[(Double, Vector)], itm: Iterator[DenseVector]) => {
        ...
        val newModel = itd.foldLeft(startPoint)(
            (sparseL2Model, dataPoint) => {
              var factor: Double = sparseL2Model._2
              val model: DenseVector = sparseL2Model._1
              if(factor < 1e-5){ // magic number to avoid numerical issue
                scal(factor, model)
                factor = 1.0
              }
              ...
              val local_loss = gradient.updateModelUseOneData(dataPoint._2, dataPoint._1, model, transStepSize, factor)

              (model, factor * (1 - thisIterStepSize * regParam))
            }
          )
  ...
  def updateModelStart(dataRDD: RDD[(Double, Vector)], bcWeights: Broadcast[DenseVector],
                       stepSize: Double, regParam: Double, gradient: Gradient, updater: Updater): RDD[DenseVector] = {
    //apply data from itt to the model itu. Note that itu only has one element, that is the model.
  ...
  /**
    * @param models
    * @param numFeatures
    * @return calculate the average of the elements inside each partition, and then distribute it back
  ...
  /**
    ...
    * @param numFeatures
    * @param numPartition
    * @return concatenate a sequence of arrays(these arrays is indexed by the key) into a whole model
    */
  def constructDenseVector(iterable: Iterable[(Int, Array[Double])], numFeatures: Int, numPartition: Int): DenseVector = {
    ...
    val average_len = numFeatures / numPartition
    var startId = 0
    var endId = 0
  ...
  /**
    * @param array
    * @param numPartition
    * @return partition an array into numPartition pieces
    */
  ...
  def runMiniBatchSGD(
    ...
    ModelAveragingSGDShuffle.runMiniBatchSGD(data, gradient, updater, stepSize, numIterations,
